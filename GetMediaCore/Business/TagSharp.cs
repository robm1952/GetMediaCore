...
namespace GetMediaCore.Business {
    internal class TagSharp {
        ...
        private Artist CreateArtisFromTagLib(TagLib.File tFile) {
            ...
            if (tFile.Tag.Performers.Length > 0) {
                foreach (var item in tFile.Tag.Performers) {
                    ...
                    _artist.ArtistSortName = CheckAndUseSortName(_artist.ArtistName);
                }
        ...
        private Int32? GetGenresId(String genName) {
            int ret = 0;
            if (String.IsNullOrEmpty(genName)) {
                ret = 20;
            }
            else {
                ret = _genres.FirstOrDefault(x => x.GenreName.Equals(genName, StringComparison.OrdinalIgnoreCase)).GenreId;
            }
            return ret;
        }
        ...
        private SongFile CreateSongFileFromTagLib(int songId, TagLib.File tFile) {
            ...
            _songFile.SongFileFqn = tFile.FileAbstraction.Name;
